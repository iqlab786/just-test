services:
  mysql-server:
    ...
    environment:
      MYSQL_ROOT_PASSWORD: iqlab@2025
      MYSQL_DATABASE: zabbix
      MYSQL_USER: zabbix
      MYSQL_PASSWORD: iqlab@2025
      TZ: Asia/Kolkata
    ports:
      - "127.0.0.1:3306:3306"
    volumes:
  ...
  zabbix-server:
    image: zabbix/zabbix-server-mysql:7.4-alpine-latest
  ...
  zabbix-web:
    ...
    environment:
      ...
      ZBX_SERVER_NAME: "Identiqa NMS"
      PHP_TZ: Asia/Kolkata
      CM_EXTERNAL_SCHEDULER: "1" 
    expose:
      - "8080"
    volumes:
      ...
      - ./config-backups:/opt/config-backups
  ...
  license-agent:
    build:
      ...
      dockerfile: Dockerfile
    container_name: license-agent
    volumes:
      - ./license/license.lic:/app/license.lic:ro
      - /var/run/docker.sock:/var/run/docker.sock
    restart: unless-stopped

  license-proxy:
    build:
      context: ./license-proxy
      dockerfile: Dockerfile
    ...
    volumes:
      - ./license/license.lic:/app/license.lic:ro
      - ./license-proxy/custom.html:/app/custom.html:ro
    restart: unless-stopped
  ...
  mysql-backup:
    ...
    environment:
      ...
      DB_DUMP_CRON: "0 2 * * 0"      # every Sunday at 2:00 AM
      DB_DUMP_RETENTION: "4c"        # keep only the last 4 backups
      DB_NAMES: zabbix
      COMPRESSION: gzip
      DB_DUMP_DEBUG: "true"
    volumes:
  ...
  configmanager-scheduler:
    build:
      context: ./modules/zabbix-config-module-main/scripts
      dockerfile: Dockerfile
    container_name: configmanager-scheduler
    restart: unless-stopped
    depends_on:
      - mysql-server
    volumes:
      - ./config-backups:/opt/config-backups   # same folder as zabbix-web above
    environment:
      DB_SERVER: mysql-server
      DB_NAME: zabbix
      DB_USER: zabbix
