...
def backup_device(conn, device: dict, script_dir: str, secret_key: str) -> bool:
    ...
    method = device.get('backup_method') or 'ssh'
    ...
    start = time.time()
    try:
        # 180s here, not 60s: backup.py's own manual-login handshake plus
        # its --More-- pagination loop (run_paged_command's own internal
        # 120s budget) can legitimately take longer than 60s on a large
        # config. A tighter ceiling here than backup.py's own worst case
        # kills perfectly good backups partway through and misreports
        # them as failures.
        ...
        exit_code = 1
        output    = 'Backup timed out after 180 seconds'
    except Exception as e:
        exit_code = 1
        output    = str(e)
